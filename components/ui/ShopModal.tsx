...
export const ShopModal: React.FC<ShopModalProps> = ({ isOpen, onClose, onBuy, onBuyGem, userCoins, tutorialStep }) => {
        ...
        <div className="p-4 overflow-y-auto min-h-0 flex-1">
          ...
          {activeTab === 'items' && (
              ...
              {itemsList.map((item) => {
                ...
                const isTutorialTarget = tutorialStep === 1 && item.id === 'planter_basic';

                return (
                  <div key={item.id} className={`bg-gray-700 p-3 rounded-lg border hover:border-gray-400 transition-all flex flex-col gap-2 group relative ${isTutorialTarget ? 'border-yellow-400 ring-4 ring-yellow-400/50 z-10 scale-105 shadow-2xl bg-gray-600' : 'border-gray-600'}`}>
          ...
          {activeTab === 'gems' && (
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  {SKUS.map((sku) => (
                      <div key={sku.id} className="bg-gray-900 border border-purple-500/30 p-4 rounded-xl flex items-center justify-between shadow-lg relative overflow-hidden">
                          ...
                          <div className="flex items-center gap-4 z-10">
                              ...
                              <div>
                                  <h3 className="font-bold text-white text-lg">{sku.amount} Gems</h3>
                  ...
                  <div className="col-span-full mt-4 p-4 bg-blue-900/20 rounded-lg border border-blue-800 text-center">
                      <p className="text-blue-200 text-sm">Use Gems to speed up crops and buy rare items!</p>
                  </div>
              </div>
          )}
