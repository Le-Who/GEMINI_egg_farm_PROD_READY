...
export const SeedBagModal: React.FC<SeedBagModalProps> = ({ isOpen, onClose, onPlant, user }) => {
      ...
      <div className="bg-gray-800 border-2 border-green-800 rounded-xl w-full max-w-md shadow-2xl overflow-hidden flex flex-col">
        ...
        <div className="flex justify-between items-center p-4 bg-green-900/50 border-b border-green-800">
          <div className="flex items-center gap-2 text-green-100">
            <Sprout size={24} />
            <h2 className="text-xl font-bold font-serif">Seed Bag</h2>
          </div>
              ...
              >
                <div className="flex justify-between items-start mb-2">
                  <div 
                    ...
                    style={{ backgroundColor: isLocked ? '#333' : `#${crop.color.toString(16).padStart(6, '0')}` }}
                  >
                ...
                <div className="flex flex-col">
                  <span className="font-bold text-gray-200">{crop.name}</span>
                  <span className="text-xs text-gray-400">{crop.growthTime}s • {crop.xpReward} XP</span>
                </div>
        ...
        <div className="p-3 bg-gray-900/80 text-center text-xs text-gray-500">
          Select a seed to plant instantly
        </div>
      </div>
    </div>
  );
