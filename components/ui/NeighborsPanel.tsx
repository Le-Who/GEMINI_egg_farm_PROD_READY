...
export const NeighborsPanel: React.FC<NeighborsPanelProps> = ({ onVisit, isOpen, onToggle }) => {
  const [neighbors, setNeighbors] = useState<NeighborProfile[]>([]);
  const [loading, setLoading] = useState(false);

  const loadNeighbors = async () => {
    setLoading(true);
    const data = await MockBackend.getNeighbors();
    setNeighbors(data);
    setLoading(false);
  };
    ...
    <div className="absolute right-20 top-20 mt-32 w-64 bg-gray-900/95 border border-gray-700 rounded-xl p-4 shadow-xl backdrop-blur-sm animate-slide-in-right z-30">
      <div className="flex justify-between items-center mb-4 border-b border-gray-700 pb-2">
        ...
        <div className="flex gap-2">
            <button onClick={loadNeighbors} className="text-gray-400 hover:text-white transition-colors" title="Refresh">
                <RefreshCw size={16} className={loading ? "animate-spin" : ""} />
            </button>
            <button onClick={onToggle} className="text-gray-400 hover:text-white transition-colors bg-gray-800 rounded-full p-1" title="Close">
                <X size={16} />
            </button>
        </div>
      </div>
      ...
      <div className="flex flex-col gap-2 max-h-[300px] overflow-y-auto">
        {neighbors.map(n => (
            <div key={n.id} className="flex items-center justify-between bg-gray-800 p-2 rounded-lg hover:bg-gray-750 transition-colors border border-gray-700">
                <div className="flex items-center gap-3">
                    {/* Avatar */}
                    {n.avatarUrl ? (
                         <img src={n.avatarUrl} alt={n.username} className="w-8 h-8 rounded-full border border-gray-600" />
                    ) : (
                    ...
                    <div className="flex flex-col">
                        <span className="font-bold text-gray-200 text-sm truncate max-w-[80px]">{n.username}</span>
