...
import { UserState, PlacedItem, ItemType, PetData, NeighborProfile, RoomType } from '../types';
import { ITEMS, CROPS, LEVELS, EGGS, SKUS, TUTORIAL_STEPS } from '../constants';
...
export const MockBackend = {
  ...
  buyItem: async (itemId: string): Promise<{ success: boolean; message?: string; newState?: UserState }> => {
  ...
  placeItem: async (itemId: string, x: number, y: number, rotation: number): Promise<{ success: boolean; message?: string; newState?: UserState }> => {
      if (!currentUserState) return { success: false };
      const state = { ...currentUserState };

      if (!state.inventory[itemId]) return { success: false, message: "No stock" };
      const items = state.rooms[state.currentRoom].items;
      if (items.some(i => i.gridX === x && i.gridY === y)) return { success: false, message: "Occupied" };

      state.inventory[itemId]--;
      items.push({
          id: crypto.randomUUID(), itemId, gridX: x, gridY: y, rotation, placedAt: Date.now(), meta: {}, cropData: null
      });
  ...
  plantSeed: async (planterId: string, cropId: string): Promise<{ success: boolean; message?: string; newState?: UserState }> => {
      if (!currentUserState) return { success: false };
      const state = { ...currentUserState };
      
      const planter = state.rooms[state.currentRoom].items.find(i => i.id === planterId);
      const crop = CROPS[cropId];
      if (!planter || !crop || state.coins < crop.seedPrice) return { success: false, message: "Error" };
  ...
  harvestOrPickup: async (x: number, y: number): Promise<any> => {
      ...
      if (config.type === ItemType.INCUBATOR && item.meta?.eggId) {
          ...
          if (elapsed >= egg.hatchTime) {
             const newPet = { 
                 id: crypto.randomUUID(), configId: egg.pool[0].petId, name: 'Pet', level: 1, xp: 0, happiness: 100, acquiredAt: Date.now() 
             };
             ...
             currentUserState = state;
             debouncedSave(state);
             return { success: true, newState: state, action: 'hatch', message: "Hatched!" };
          }
      ...
      if (item.cropData) {
          ...
          if (elapsed >= crop.growthTime * 1000) {
              ...
              currentUserState = state;
              debouncedSave(state);
              return { success: true, newState: state, action: 'harvest', reward: crop.sellPrice };
          }
  ...
  waterNeighborPlant: async (neighborId: string, plantId: string) => ({ success: true, message: "Watered!" }),
  ...
  useConsumable: async (itemId: string, x: number, y: number) => {
      ...
      if ((state.inventory[itemId] || 0) > 0) {
          ...
          if (itemId === 'fertilizer') {
             const item = state.rooms[state.currentRoom].items.find(i => i.gridX === x && i.gridY === y);
      ...
      return { success: false, message: "No stock or invalid target" };
  },
...
const checkTutorial = (state: UserState, action: string, targetId?: string) => {
    ...
    if (currentStepConfig && currentStepConfig.trigger === action) {
        if (currentStepConfig.targetId && currentStepConfig.targetId !== targetId) return;
        state.tutorialStep++;
        if (state.tutorialStep >= TUTORIAL_STEPS.length) state.completedTutorial = true;
    }
};
